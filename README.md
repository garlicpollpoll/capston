...
### 3. Redis Standalone -> Redis Sentinel
기존 ver.2에서 Redis를 이용해 RDBMS의 부하를 30퍼센트 줄여 정적인 데이터의 읽기 연산의 부담을 줄여 문제를 개션하였습니다. 

Redis의 standalone의 상태에선 읽기 요청이 늘어나면 Redis의 부하로 이어지고 이는 Redis의 장애로 이어질 수 있습니다. 이후 Redis가 처리하던 읽기 연산을 RDBMS가 부담하게 되어 RDBMS의 장애로 이어질 수 있다는 문제를 발생할 수 있습니다. 

이 문제를 해결하기 위해 Redis Sentinel을 이용해 고가용성을 챙기면서 마스터 노드의 장애시 데이터 유실에 대한 문제를 해결할 수 있었습니다. 

Redis의 고가용성을 위한 전략 중 Sentinel과 Cluster중 Sentinel을 선택한 이유는 Sharding이 어느수준에서 필요한 단계인지 정확하게 인지하지 못해 선택하는 것에 망설임이 있었습니다. 

또한, Sharding을 선택할 때는 수직적인 확장, 서버 개수의 증가 등을 충분히 진행한 뒤 최후의 방법으로 선택하라는 MongoDB 공식 문서에 의거해 Sentinel을 선택하게 되었습니다. 

Sentinel을 통해 읽기 연산을 분산시키면서 고가용성을 챙길 수 있게 되었습니다. 

또한, Sentinel의 Notification-Script 기능을 이용해 슬랙과 연동하여 장애상황에 대한 서버의 상황을 실시간으로 알람을 받을 수 있게 설계하였습니다. 
