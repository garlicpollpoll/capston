...
public class ElasticsearchRepositoryTest {
    ...
    public void test() throws Exception {
        ...
        String name = "3";
        QueryBuilder queryBuilder = QueryBuilders.matchQuery("view_name", name);
        SearchHits<TestData> search = template.search(new NativeSearchQueryBuilder()
                .withQuery(queryBuilder)
                .withPageable(PageRequest.of(0, 10))
                .build(), TestData.class);
        List<TestData> collect = search.get().map(SearchHit::getContent).collect(Collectors.toList());
        TestData testData = collect.get(0);
        System.out.println("ViewName : " + testData.getView_name());
        System.out.println("ItemName : " + testData.getItem_name());
        System.out.println("Price : " + testData.getPrice());
        //when
